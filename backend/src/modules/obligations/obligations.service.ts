...
} from 'src/domain/obligation/obligation';
...
export class ObligationsService {
  ...
  async createObligation(createObligationDto: CreateObligationDto) {
    ...
    return {
      status: 'success',
      data: { ...saved, companyTaxId: maskCompanyTaxId(saved.companyTaxId) },
    };
  }
  ...
  async findAll(companyTaxId: string) {
    ...
    const obligations = await this.prismaService.obligation.findMany({
      where: { companyTaxId, enabled: true },
      orderBy: { dueDate: 'asc' },
    });
    ...
    return {
      ...
      data: obligations.map((obligation) => ({
        ...
        overdue: isOverdue({
          ...
          status: obligation.status as Status,
        }),
      })),
    };
  }
  ...
  async findOne(id: string) {
    ...
    const obligation = await this.prismaService.obligation.findUnique({
  ...
  async update(id: string, updateObligationDto: UpdateObligationDto) {
    ...
    try {
      ...
      return {
        ...
        data: {
          ...obligationUpdated,
          companyTaxId: maskCompanyTaxId(obligationUpdated.companyTaxId),
        },
      };
    } catch (error: unknown) {
      if (
  ...
  async disable(id: string) {
    ...
    try {
      const deleted = await this.prismaService.obligation.update({
        where: { id, version: obligation.version },
        data: {
          enabled: false,
          deletedAt: new Date(),
      ...
      return {
        ...
        data: {
          ...deleted,
          companyTaxId: maskCompanyTaxId(deleted.companyTaxId),
        },
      };
    } catch (error: unknown) {
      if (
  ...
  async updateStatus(id: string, updateStatusDto: ChangeObligationStatusDto) {
    ...
    const result = updateObligationStatus(
      obligation as Obligation,
  ...
  async getHistory(id: string) {
    ...
    const history = await this.prismaService.obligationStatusHistory.findMany({
